...
def send(printer,data):
    try: return output(printer.print_ticket(data))
...
def text(payload: TextJob,request: Request):
    printer,config=svc(request)
...
def character_table_test(request: Request):
    printer,config=svc(request); encoding=config.load()["encoding"]
    ...
    return send(printer,b.feed(4).build())
