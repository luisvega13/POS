...
import {api} from '../api';
import type {CartLine,PaymentMethod,Permissions,Product,UserRole} from '../types';
...
export function PosPage({products,cashOpen,role,permissions,onReload,onNotice}:{products:Product[];cashOpen:boolean;role:UserRole;permissions:Permissions;onReload:()=>void;onNotice:(message:string,error?:boolean)=>void}){
 const [query,setQuery]=useState(''),[category,setCategory]=useState(''),[cart,setCart]=useState<Map<number,CartLine>>(new Map()),[paying,setPaying]=useState(false),[method,setMethod]=useState<PaymentMethod>('cash'),[split,setSplit]=useState(false),[splitCash,setSplitCash]=useState(''),[splitCard,setSplitCard]=useState(''),[splitTransfer,setSplitTransfer]=useState(''),[received,setReceived]=useState(''),[tip,setTip]=useState(''),[tipMode,setTipMode]=useState<ValueMode>('amount'),[tipMethod,setTipMethod]=useState<PaymentMethod>('cash'),[discount,setDiscount]=useState(''),[discountMode,setDiscountMode]=useState<ValueMode>('amount'),[discountReason,setDiscountReason]=useState(''),[saving,setSaving]=useState(false);
 const categories=[...new Set(products.map(product=>product.category))],filtered=products.filter(product=>(!category||product.category===category)&&product.name.toLowerCase().includes(query.toLowerCase())),lines=[...cart.values()],total=lines.reduce((sum,line)=>sum+Number(line.product.price)*line.quantity,0),discountValue=calculated(total,discount,discountMode),discountedTotal=Math.max(0,total-discountValue),serviceCharge=permissions.include_tip_in_ticket?Math.round(discountedTotal*permissions.service_charge_percent)/100:0,tipValue=permissions.include_tip_in_ticket?serviceCharge:calculated(discountedTotal,tip,tipMode),splitCents=cents(splitCash)+cents(splitCard)+cents(splitTransfer),totalCents=cents(discountedTotal),splitSum=splitCents/100,splitBalance=(totalCents-splitCents)/100,cashPayment=split?cents(splitCash)/100:(method==='cash'?discountedTotal:0),cashDue=cashPayment+(tipValue>0&&tipMethod==='cash'?tipValue:0),discountBlocked=role==='cashier'&&permissions.cashier_discount_mode==='denied';
 const setQty=(product:Product,delta:number)=>setCart(current=>{const next=new Map(current),quantity=(next.get(product.id)?.quantity||0)+delta;quantity>0?next.set(product.id,{product,quantity}):next.delete(product.id);return next});
 async function confirm(){if(split&&splitCents!==totalCents)return onNotice('La suma de los pagos debe coincidir con el total de la venta',true);if(discountMode==='percent'&&Number(discount)>100)return onNotice('El porcentaje de descuento no puede superar 100%',true);if(discountValue>total)return onNotice('El descuento no puede superar el consumo',true);if(discountValue>0&&!discountReason.trim())return onNotice('Indica el motivo del descuento',true);let discount_password:string|undefined;if(discountValue>0&&role==='cashier'&&permissions.cashier_discount_mode==='password'){discount_password=window.prompt('Ingresa la contraseña operativa para autorizar el descuento')||undefined;if(!discount_password)return}if(cashDue>0&&cents(received)<cents(cashDue))return onNotice('El efectivo recibido es menor al total en efectivo',true);const payments=split?([['cash',splitCash],['card',splitCard],['transfer',splitTransfer]] as [PaymentMethod,string][]).filter(([,value])=>cents(value)>0).map(([paymentMethod,value])=>({method:paymentMethod,amount:cents(value)/100})):[];setSaving(true);try{const result=await api.createSale({items:lines.map(line=>({product_id:line.product.id,quantity:line.quantity})),payment_method:method,payments,amount_received:cashDue>0?received:null,tip_amount:tipValue,tip_method:tipMethod,discount_amount:discountValue,discount_reason:discountReason,discount_password,print_ticket:true});setCart(new Map());setPaying(false);setTip('');setReceived('');setDiscount('');setDiscountReason('');setSplit(false);setSplitCash('');setSplitCard('');setSplitTransfer('');onNotice(result.message,result.print_success===false);onReload()}catch(error){onNotice((error as Error).message,true)}finally{setSaving(false)}}
 return <div className="pos-grid">
...
</button>)}{!filtered.length&&<div className="empty">No hay productos que coincidan.</div>}</div>
</section>
...
<button className="text-button" onClick={()=>setCart(new Map())}>Limpiar</button>
</header>
...
</article>)}{!lines.length&&<div className="empty">
<ShoppingBag/>
...
<button className="primary full" disabled={!lines.length} onClick={()=>cashOpen?setPaying(true):onNotice('Debes abrir caja antes de vender',true)}>Cobrar</button>
</div>
</aside>{paying&&<Modal title="Cobrar venta" onClose={()=>setPaying(false)}>
...
<input disabled={discountBlocked} type="number" min="0" max={discountMode==='percent'?100:total} step="0.01" value={discount} onChange={event=>setDiscount(event.target.value)} placeholder="0.00"/>
</span>
...
</label>}<button className="primary full" disabled={saving} onClick={()=>void confirm()}>{saving?'Registrando…':'Confirmar cobro'}</button>
</Modal>}</div>;
}
